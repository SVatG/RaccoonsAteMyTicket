...
{
	Bitmap *bitmap=(Bitmap *)malloc(sizeof(Bitmap)+SizeOfBitmapDataWithWidthAndHeight(width,height));
	if(!bitmap) return NULL;
	InitialiseBitmap(bitmap,width,height,BytesPerRowForWidth(width),(Pixel *)&bitmap[1]);
	return bitmap;
}

static inline Bitmap *AllocateSubBitmapAsReference(Bitmap *bitmap,int x,int y,int width,int height)
{
...
static inline Bitmap *AllocateSubBitmapAsCopy(const Bitmap *bitmap,int x,int y,int width,int height)
{
